...
hardware_interface::CallbackReturn MyRobotSystem::on_init(
  const hardware_interface::HardwareInfo & info)
{
  ...
  // Teknofest-AGV: Sol ve sağ encoder ayrı değerler
  ...
  {
    RCLCPP_INFO(rclcpp::get_logger("MyRobotSystem"), "PID values not supplied, using defaults.");
  }
  ...
  {
    // MyRobotSystem has exactly two states and one command interface on each joint
    if (joint.command_interfaces.size() != 1)
    {
      RCLCPP_FATAL(
        rclcpp::get_logger("MyRobotSystem"),
...
std::vector<hardware_interface::StateInterface> MyRobotSystem::export_state_interfaces()
{
...
std::vector<hardware_interface::CommandInterface> MyRobotSystem::export_command_interfaces()
{
...
hardware_interface::CallbackReturn MyRobotSystem::on_configure(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(rclcpp::get_logger("MyRobotSystem"), "Configuring ...please wait...");
  if (comms_.connected())
  {
    comms_.disconnect();
  }
  comms_.connect(cfg_.device, cfg_.baud_rate, cfg_.timeout_ms);
  RCLCPP_INFO(rclcpp::get_logger("MyRobotSystem"), "Successfully configured!");
...
hardware_interface::CallbackReturn MyRobotSystem::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(rclcpp::get_logger("MyRobotSystem"), "Activating ...please wait...");
  if (!comms_.connected())
  {
    return hardware_interface::CallbackReturn::ERROR;
  }
  ...
  RCLCPP_INFO(rclcpp::get_logger("MyRobotSystem"), "Successfully activated!");

  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn MyRobotSystem::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
...
PLUGINLIB_EXPORT_CLASS(
  my_robot_hardware::MyRobotSystem, hardware_interface::SystemInterface)
